...
HTMLParser.prototype.appendText = function(token) {
  var content = token.chars.replace(/&#?[a-zA-Z0-9]+;/g, function (match) {
    var charCode;
    if (match.indexOf('&#x') === 0) {
      // HEX - convert to DEC
      charCode = parseInt(match.slice(3, -1), 16);
    } else if (match.indexOf('&#') === 0) {
      // DEC
      charCode = parseInt(match.slice(2, -1), 10);
    } else {
      // ALPHA
      charCode = entityMap[match.slice(1, -1)];
    }
