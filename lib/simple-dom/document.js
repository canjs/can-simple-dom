...
function warnMissingMethod(name, methodOrNot = true){
	var hasWarned = false;
	var warning = "document."+name+" is not implemented in can-simple-dom with witch SSR occurs.  Please make sure this code runs after the document has finished loading, with guide at bitballs (turning off SSR) under donejs.com."
	if (methodOrNot){
		hasWarned = true
...
warnMissingMethod("querySelector")
warnMissingMethod("querySelectorAll")
